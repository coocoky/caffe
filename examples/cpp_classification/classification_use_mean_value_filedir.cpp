...
class Classifier {
 public:
  Classifier(const string& model_file,
             ...
             const string& mean_value_or_file,
 ...
 private:
  ...
  void SetMean_use_mean_value(const string& mean_value);
...
Classifier::Classifier(const string& model_file,
  ...
  SetMean_use_mean_value(mean_value_or_file);
...
int main(int argc, char** argv) {
  string imgfilename;
    ...
    std::ofstream inFile;
    inFile.open(savepath, std::ios::trunc | std::ios::out);
    ...
    while (getline(fin, imgfilename)) //依次读入文件中每个图片路径
    {
      ...
      //存
      inFile << imgfilename << "\t" << predictions[0].second << "\t" << predictions[0].first << "\t" << predictions[1].second << "\t" << predictions[1].first << "\t" << predictions[2].second << "\t" << predictions[2].first << std::endl;
      std::cout << "Successwrite record!" << std::endl;//屏幕输出每条记录写入成功信息  
    }
   inFile.close();//关闭文件 
}
#else
